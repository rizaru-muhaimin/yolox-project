...
def _labels() -> list[str]:
    data_dir = Path(
        os.environ.get("WORKSHOP_DATA_DIR", str(ROOT / "datasets" / "workshop_tools"))
    ).expanduser().resolve()
...
def _checkpoint_state(path: Path, torch):
    ...
    try:
        # YOLOX stores AP metrics as NumPy float64 alongside model weights.
...
def main() -> int:
    ...
    exp = get_exp(str(ROOT / "exps" / "workshop_yolox_s.py"), None)
